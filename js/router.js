...
async function loadBewegungsdaten(state) {
    try {
        ...
        _syncScaleModes();
    } catch (err) { console.error('Bewegungsdaten:', err); }
}
...
async function loadMonitoring() {
    try {
        ...
        renderMonitoring({ live: {}, history: `Fehler: ${err.message}` });
    }
}
...
async function loadChangelog() {
    try {
        ...
        const c = document.getElementById('changelog-list');
        if (c) c.innerHTML = `<p style="color:#E20074">Fehler: ${err.message}</p>`;
    }
}
    ...
    try {
        if (idtype === 'bewegung') {
            ...
            for (let pg = 1; pg <= 20 && !found; pg++) {
                ...
                found = data.items?.find((r) => String(r.id) === String(id));
...
function goToPage(page) { updateUrl(null, { p: page }); }
...
function _syncScaleModes() {
    ...
    document.querySelectorAll('scale-button,scale-tag,scale-table,scale-text-field,scale-modal,[mode]')
        .forEach((el) => { el.setAttribute('mode', mode); if ('mode' in el) el.mode = mode; });
}

window.addEventListener('popstate', render);

document.addEventListener('scale-close', (e) => {
    if (e.target?.id === 'detail-modal') updateUrl(null, { id: null, idtype: null });
});

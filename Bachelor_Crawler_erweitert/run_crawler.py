...
def _fetch_targets(conn, max_targets: int, prio_region: str) -> list:
    """
    Liest Crawl-Ziele aus crawl_targets.
    Sortierung: last_scanned ASC NULLS FIRST (unbearbeitete zuerst).
    Optional: Prio-Region wird vorgezogen.
    """
...
def _update_last_scanned(conn, ags: str) -> None:
    with conn.cursor() as cur:
        cur.execute(
            "UPDATE crawl_targets SET last_scanned = NOW() WHERE ags = %s",
            (ags,)
        )
    conn.commit()
...
def run_all() -> None:
    max_pages   = DEFAULT_CONFIG.get('max_pages', 50)
    max_targets = int(os.getenv('CRAWLER_MAX_TARGETS', '0'))  # 0 = alle
    prio_region = os.getenv('PRIO_REGION', '')
    sleep_sec   = float(os.getenv('CRAWLER_SLEEP_BETWEEN_TARGETS', '1.0'))
    ...
    # Direkte psycopg2-Verbindung für crawl_targets (read/update)
    import psycopg2
    conn = psycopg2.connect(DEFAULT_CONFIG['db_url'])

    targets = _fetch_targets(conn, max_targets, prio_region)
    total   = len(targets)
    logger.info('Starte Crawler | %d Kommunen | max_pages=%d | prio_region=%s',
    ...
    conn.close()
    logger.info('=== Crawler abgeschlossen: %d/%d Kommunen verarbeitet ===', idx, total)

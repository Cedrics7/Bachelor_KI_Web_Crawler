...
# ---------------------------------------------------------------
# LLM-Kategorien (Ziel-Kategorien für Massnahmen-Klassifikation)
# Struktur: {Kategoriename: [Synonyme/Schluesselbegriffe, ...]}
# Überschreibbar per .env als JSON:
#   CRAWLER_ZIEL_KATEGORIEN='{"Sanierung": ["Sanierungsgebiet"], ...}'
# ---------------------------------------------------------------
def _parse_kategorien(val: str | None, default: dict) -> dict:
    if not val:
        return default
    try:
        ...
        if isinstance(parsed, dict):
            return parsed
        logger.warning('Config: CRAWLER_ZIEL_KATEGORIEN ist kein JSON-Objekt - Default wird verwendet.')
    except (json.JSONDecodeError, TypeError):
        logger.warning('Config: CRAWLER_ZIEL_KATEGORIEN ist kein valides JSON - Default wird verwendet.')
    return default


_DEFAULT_ZIEL_KATEGORIEN: dict = {
...
ZIEL_KATEGORIEN: dict = _parse_kategorien(
    os.getenv('CRAWLER_ZIEL_KATEGORIEN'), _DEFAULT_ZIEL_KATEGORIEN
)
...
DEFAULT_CONFIG: Dict[str, Any] = {
    ...
    'ziel_kategorien':          ZIEL_KATEGORIEN,
...
logger.info('  ziel_kategorien      : %s', ', '.join(ZIEL_KATEGORIEN.keys()))

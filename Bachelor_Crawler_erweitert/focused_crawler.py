...
DEFAULT_CONFIG: Dict = {
    'user_agent': 'Mozilla/5.0 (BachelorCrawlerVollstaendig)',
    'timeout_seconds': 12,
    'max_redirects': 5,
...
class FocusedCrawler:
    def __init__(self, config: Optional[Dict] = None, run_id: Optional[str] = None, logger: Optional[CrawlerLogger] = None) -> None:
        self._config = {**DEFAULT_CONFIG, **(config or {})}
        self._domain_model = DomainModel()
        self._classifier = RelevanceClassifier(self._domain_model, self._config['relevance_threshold'])
        self._prioritizer = LinkPrioritizer(self._domain_model, self._config['priority_threshold'])
        self._privacy = PrivacyGuard()
        self._robots = RobotsChecker(self._config['user_agent'], self._config['robots_timeout']) if self._config.get('robots_respect', True) else None
        self._http_headers = {'User-Agent': self._config['user_agent']}
        self._logger = logger or CrawlerLogger(run_id=run_id or 'vollstaendig_run', log_dir=self._config['log_dir'])
        self._owns_logger = logger is None

    def crawl(self, start_url: str, max_pages: Optional[int] = None, reference_corpus_size: Optional[int] = None) -> Tuple[List[CrawlResult], EvaluationReport]:
        limit = max_pages or self._config['max_pages']
        evaluator = CrawlEvaluator(start_url=start_url, reference_corpus_size=reference_corpus_size)
        ...
        first_request = True
        with httpx.Client(follow_redirects=True, max_redirects=self._config['max_redirects'], headers=self._http_headers, timeout=self._config['timeout_seconds']) as client:
            while queue and len(results) < limit:
                ...
                if curr_base in visited_urls or self._privacy.is_sensitive_url(curr_url):
                    evaluator.add_skipped(); continue
                visited_urls.add(curr_base)
                mem_mb = self._get_rss_mb()
                if mem_mb > self._config['ram_warn_mb']:
                    self._logger.info('SYSTEM', 'RAM-Warnung', rss_mb=round(mem_mb,1), queue_size=len(queue))
                if self._robots and not self._robots.is_allowed(curr_url):
                    evaluator.add_robots_blocked(); self._logger.privacy(curr_url, 'ROBOTS_DISALLOWED', 'robots.txt blockiert'); continue
                delay = max(self._config['crawl_delay_default'], self._robots.get_crawl_delay(curr_url) if self._robots else 0.0)
                if self._robots: self._robots.wait_for_crawl_delay(curr_url)
                elif delay: time.sleep(delay)
                t0 = time.time()
                try:
                    resp = client.get(curr_url)
                except Exception as e:
                    self._logger.error('HTTP', f'Request-Fehler: {str(e)[:80]}', url=curr_url); continue
                if self._robots: self._robots.record_access(curr_url)
                fetch_ms = (time.time() - t0) * 1000
                final_url, final_domain = str(resp.url), urlparse(str(resp.url)).netloc
                if first_request:
                    first_request = False
                    if self._strip_www(final_domain) != self._strip_www(base_domain) and self._is_vg_redirect(base_domain, final_url):
                ...
                if self._strip_www(final_domain) != self._strip_www(effective_domain):
                    self._logger.privacy(curr_url, 'DOMAIN_GUARD', f'Externe Domain: {final_domain}'); continue
                ...
                if is_pdf:
                    ...
                    if self._config.get('js_rendering') and self._is_js_rendered(raw_html):
                        ...
                        if js_html and len(js_html) > len(raw_html):
                            raw_html = js_html
                    text, blocks, new_links = self._extract_html(raw_html, curr_url, effective_domain, effective_start_path)
                    scored_links = self._prioritizer.score_links(new_links, page_text=text)
                    for sl in scored_links:
                        if len(queue) >= self._config['max_queue']: break
                        target_base = self._get_url_base(sl.url)
                        if target_base in visited_urls: continue
                        entry = (sl.url, sl.anchor_text, '')
                        if sl.is_priority or sl.is_pdf: queue.insert(0, entry)
                ...
                result = CrawlResult(curr_url, text, relevance, content_hash, is_pdf, resp.status_code, blocks, round(fetch_ms,1))
                results.append(result)
                evaluator.add_result(relevance, is_pdf=is_pdf)
        report = evaluator.get_report()
        self._logger.evaluation(report.to_dict(), label='FOCUSED')
        if self._owns_logger: self._logger.close()
        return results, report

    def _extract_html(self, html: str, base_url: str, effective_domain: str, effective_start_path: str) -> Tuple[str, List[str], List[Tuple[str, str, str]]]:
        soup = BeautifulSoup(html, 'html.parser')
        for tag in soup.find_all(['script','style','nav','footer','header']): tag.decompose()
        ...
        for block_tag in soup.find_all(['div','section','article','main']):
            ...
            if len(block_text) > 100:
                ...
                if score > 0.05: blocks.append(block_text[:500])
        bs_links = []
        for a_tag in soup.find_all('a', href=True):
            href = a_tag.get('href', '')
            full_url = urljoin(base_url, href)
            parsed = urlparse(full_url)
            if self._strip_www(parsed.netloc) != self._strip_www(effective_domain): continue
            if effective_start_path and not parsed.path.startswith(effective_start_path): continue
        ...
        for item in bs_links + regex_links:
            if item[0] not in seen:
                seen.add(item[0]); links.append(item)
        soup.decompose()
    ...
    def _is_js_rendered(self, html: str) -> bool:
        if len(html.strip()) < self._config.get('js_min_chars', 500): return True
        html_lower = html.lower()
        return any(m in html_lower for m in ['<noscript>', 'id="root"', "id='root'", 'id="app"', "id='app'", 'data-reactroot', 'ng-version', 'data-v-app'])

...
link_prioritizer.py
===================
CPE-basierte Link-Priorisierung für den Focused Crawler.
...
Links mit höherem CPE-Score werden priorisiert in die Queue eingereiht.

Neu gegenüber scraper_js:
    - Ankertext-Bewertung (nicht nur URL-Keywords)
    - Linkkontext-Bewertung (umgebender Text des <a>-Tags)
    - URL-Pattern-Scoring (Pfadsegmente gegen Domänen-Keywords)
    - Gewichtete Kombination aller vier Quellen
...
class LinkPrioritizer:
    ...
    die sortierten Links zurück (höchster Score zuerst).

    Verwendung:
        prioritizer = LinkPrioritizer()
        scored_links = prioritizer.score_links(
            links=[...],          # Liste (url, anchor_text, context_text)
            page_text="...",       # Gesamttext der aktuellen Seite
        )
        # scored_links[0] = Link mit höchstem CPE-Score
    """

    # CPE-Gewichte (w1..w4): Summe = 1.0
    # Ankertext erhält höchstes Gewicht, da er häufig den Zielinhalt beschreibt
    ...
    def score_links(
        self,
        links: List[Tuple[str, str, str]],  # (url, anchor_text, context_text)
        page_text: str = "",
    ) -> List[ScoredLink]:
        """
        Bewertet eine Liste von Links und gibt sie sortiert nach CPE-Score zurück.
        ...
        Returns:
            Sortierte Liste von ScoredLink (höchster Score zuerst)
        """
        ...
        # Sortierung: PDFs zuerst, dann nach CPE-Score
        scored.sort(key=lambda x: (x.is_pdf, x.cpe_score), reverse=True)
        return scored

    def score_url_only(self, url: str) -> float:
        """Schneller URL-Score ohne Ankertext/Kontext (für Queue-Vorsortierung)."""
    ...
    def _score_single(
        ...
        # PDF-Bonus: PDFs erhalten URL-Score-Boost
        if is_pdf:
    ...
    def _score_url(self, url: str) -> float:
        ...
        gegen die Domänen-Keywords.
        Beispiel: /bebauungsplan/2024/ → hoher Score
        """
        parsed = urlparse(url)
        # Pfadsegmente und Dateiname als Text zusammensetzen
